...
class ArticlesModelAgSearchagosms extends JModelList
{
	...
	function __construct()
	{
		...
		$this->module_helper = new modAgosmsSearchagosmsHelper;
		...
		if (isset($this->module_params->savesearch) && $this->module_params->savesearch && JFactory::getSession()->get("SaveSearchValues") && $_GET['applySaved']
			...
			$skip = ["option", "task", "view", "Itemid", "search_mode", "field_id", "field_type", "initial"];
	...
	function getItems($total = false)
	{
		if (isset($this->module_params->georestrict) && $this->module_params->georestrict) {
			...
			foreach ($filterresults as $filterresult) {
				$jcFields = FieldsHelper::getFields('com_agosms.agosm', $filterresult, true);
...
/*todo				$fields = FieldsHelper::getFields('com_agosms.agosm', $item, true);
	...
	/*	$query .= " LEFT JOIN #__contentitem_tag_map AS tm 
		...
		$query .= " AND (i.publish_up  <= '{$time}' OR i.publish_up IS NULL) AND (i.publish_down > '{$time}' OR i.publish_down IS NULL)";
	...
	function getSearchQuery()
	{
		...
		if (JFactory::getApplication()->input->get("keyword")) {
			...
			if (array_key_exists('match', $_GET) && $_GET['match'] == 'any') {
				...
				$query .= "  OR UPPER(i.description) LIKE '%{$keyword}%'";
		...
		// Fields search
		require_once JPATH_SITE . '/modules/mod_agosms_searchagosms/helper.php';
		$module_helper = new modAgosmsSearchagosmsHelper;
	...
	function getPagination()
	{
		...
		foreach ($_GET as $param => $value) {
			if (in_array($param, ["id", "start", "option", "view", "task"])) {
				continue;
			}
	...
	function execPlugins(&$item)
	{
		...
		JFactory::getApplication()->triggerEvent('onContentPrepare', ['com_agosms.category', &$item, &$item->params, 0]);
		//$dispatcher->trigger('onContentPrepare', ['com_agosms.category', &$item, &$item->params, 0]);
	...
	/*	$item->introtext = $item->text;
		...
		$results = $dispatcher->trigger('onContentBeforeDisplay', ['com_agosms.category', &$item, &$item->params, 0]);
		$item->event->beforeDisplayContent = trim(implode("\n", $results));

		$results = $dispatcher->trigger('onContentAfterTitle', ['com_agosms.category', &$item, &$item->params, 0]);
		$item->event->afterDisplayTitle = trim(implode("\n", $results));

		$results = $dispatcher->trigger('onContentAfterDisplay', ['com_agosms.category', &$item, &$item->params, 0]);
	...
	function getStatsListPagination()
	{
		...
		foreach ($_GET as $param => $value) {
			if (in_array($param, ["id", "start", "option", "view", "task", "limit", "featured"])) {
				continue;
			}
