document.addEventListener(
  ...
  function () {
    ...
    [].forEach.call(leafletmapsMod, function (element) {
      ...
      var thunderforestmaptype = element.getAttribute(
        "data-thunderforestmaptype"
      );
      ...
      var esrireversegeocoding = element.getAttribute(
        "data-esrireversegeocoding"
      );
      var geocodercollapsed =
        element.getAttribute("data-geocodercollapsed") === "true";
      var geocoderposition = element.getAttribute("data-geocoderposition");
      var expand = element.getAttribute("data-expand");
      var dataEsrigeocoderopengetaddress =
        element.getAttribute("data-esrigeocoderopengetaddress") === "true";
      var showgeocoderesri = element.getAttribute("data-showgeocoderesri");
      var positionesrigeocoder = element.getAttribute(
        "data-positionesrigeocoder"
      );
      var esrigeocoderzoomToResult =
        element.getAttribute("data-esrigeocoderzoomToResult") === "true";
      var esrigeocoderuseMapBounds =
        element.getAttribute("data-esrigeocoderuseMapBounds") === "true";
      var esrigeocodercollapseAfterResult =
        element.getAttribute("data-esrigeocodercollapseAfterResult") === "true";
      var esrigeocoderexpanded =
        element.getAttribute("data-esrigeocoderexpanded") === "true";
      var esriallowMultipleResults =
        element.getAttribute("data-esriallowMultipleResults") === "true";
      var showrouting_simple = element.getAttribute("data-showrouting-simple");
      if (showrouting_simple === "1") {
        var routesimpleposition = element.getAttribute(
          "data-route-simple-position"
        );
        var routesimpletarget = element.getAttribute(
          "data-route-simple-target"
        );
        var routesimplerouter = element.getAttribute(
          "data-route-simple-router"
        );
        var routesimplerouterkey = element.getAttribute(
          "data-route-simple-routerkey"
        );
      }
      var showrouting = element.getAttribute("data-showrouting");
      if (showrouting === "1") {
        var routingstart = element
          .getAttribute("data-routingstart")
          .split(",", 3);
        ...
        var routewhiledragging = element.getAttribute(
          "data-routewhiledragging"
        );
      }
      ...
      if (showcomponentpin === "1") {
        var specialcomponentpins = JSON.parse(
          element.getAttribute("data-specialcomponentpins")
        );
      }
      ...
      var owngooglegesturetext = element.getAttribute(
        "data-owngooglegesturetext"
      );
      ...
      if (true) {
        ...
        if (
          JSON.parse(sessionStorage.getItem("mapState")) &&
          savestate === "1"
        ) {
          window["mysearchmap" + moduleId].fitBounds(mapState.bounds);
        } else {
          window["mysearchmap" + moduleId].fitBounds(
            clustermarkers.getBounds()
          );
        }
